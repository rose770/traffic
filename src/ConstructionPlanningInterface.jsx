...
// Set VITE_API_URL to '' (empty string) when the frontend and backend are served
// from the same origin (e.g. Express serving the built frontend on Render) so
// requests go to relative paths like /api/permits instead of a hardcoded host.
// Falls back to localhost for local development with `npm run dev`.
...
import ActiveDetourMonitoringModal from './ActiveDetourMonitoringModal';
import DetourClosureModal from './DetourClosureModal';
...
const dict = {
  ar: {
    dir: 'rtl',
    portalTitle: 'البوابة البلدية لتخطيط وتراخيص السلامة المرورية',
    ...
    inspectorSig: 'توقيع واعتماد الإدارة المختصة بالبلدية',
  ...
  en: {
    dir: 'ltr',
    portalTitle: 'Municipal Traffic Safety & Permit Licensing Portal',
    ...
    back: 'Back',
    next: 'Next',
    submit: 'Verify Design & Submit to Amanah Licensing',
    print: 'Print Compliance Certificate',
    edit: 'Edit Detour Specifications',
    ksa: 'Kingdom of Saudi Arabia',
    amanaDept: 'Amanah Al-Madinah Region',
    ...
    inspectorSig: 'Amanah Licensing Authority Signature',
    ...
    <div className="min-h-screen bg-slate-100 flex flex-col justify-center items-center p-4" dir={language === 'ar' ? 'rtl' : 'ltr'}>
      <div className="max-w-md w-full bg-white rounded-2xl shadow-xl overflow-hidden border border-brand-green/20">
        <div className="bg-brand-green p-6 text-center text-white">
          <Shield className="w-12 h-12 mx-auto mb-3 text-brand-gold" />
          ...
          {!isLogin && (
            <div>
              ...
              >
                ...
                <option value="inspector">{language === 'ar' ? 'مفتش بلدي' : 'Municipal Inspector'}</option>
              </select>
            </div>
          )}
        ...
        {isLogin && (
          <div className="bg-slate-50 p-4 border-t border-slate-100 text-xs text-slate-500 text-center flex flex-col gap-1">
            <span>{language === 'ar' ? 'الاختصارات السريعة للتجربة:' : 'Quick Test Logins:'}</span>
            <div className="flex justify-center gap-4 mt-1">
              <button onClick={() => {setUsername('contractor'); setPassword('pass123');}} className="hover:text-brand-primary font-mono underline">contractor</button>
              <button onClick={() => {setUsername('inspector'); setPassword('pass123');}} className="hover:text-brand-primary font-mono underline">inspector</button>
            </div>
          </div>
        )}
      </div>
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm">
      <div className="bg-white rounded-2xl w-full max-w-lg overflow-hidden shadow-2xl animate-fade-in" dir={language === 'ar' ? 'rtl' : 'ltr'}>
        ...
        <div className="p-6 space-y-4 text-left" dir={language === 'ar' ? 'rtl' : 'ltr'}>
          ...
          <button onClick={() => { alert(language === 'ar' ? 'تم إرسال التقرير بنجاح للأمانة.' : 'Report submitted successfully to Amanah.'); onClose(); }} className="w-full bg-brand-primary text-brand-dark hover:bg-brand-gold font-bold py-3 rounded-xl mt-4 transition-colors">
...
const ConstructionPlanningInterface = () => {
  ...
  const [expandedPanels, setExpandedPanels] = useState({
    ...
    p3_auditor: false,
    p3_checklist: false,
    p3_preapproval: false,
    p3_fieldinspection: false,
    p3_photos: false,
    p3_approval_workflow: false
  });
  ...
  const [formData, setFormData] = useState({
    ...
    clientNameEn: 'Al-Madinah Development Authority',
    ...
    projectOwnerAr: 'أمانة منطقة المدينة المنورة',
    projectOwnerEn: 'Amanah Al-Madinah',
    ...
    lightingAlertChannel: 'amanah_platform',
  ...
  const fetchPermits = async () => {
    try {
      const res = await fetch(`${API_URL}/api/permits`);
  ...
  const handleUpdatePermitStatus = async (status) => {
    ...
    try {
      const res = await fetch(`${API_URL}/api/permits/${targetId}`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
  ...
  const handleExportCAD = async () => {
    ...
    try {
      ...
      const res = await fetch(`${API_URL}/api/generate-cad`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          lat: centerLat,
          ...
          detourNodes,
          boundaryPoints,
          projectName: formData.projectNameAr || formData.projectNameEn || 'Madinah Detour Site'
        })
      });
      ...
      a.href = url;
      a.download = `madinah_detour_site_${Date.now()}.dxf`;
  ...
  const handleSavePeriodicAudit = async (auditData) => {
    try {
      await fetch(`${API_URL}/api/permits/${auditData.permitId}/periodic-inspections`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
  ...
  const handleSaveClosureMinutes = async (closureData) => {
    try {
      await fetch(`${API_URL}/api/permits/${closureData.permitId}/closure`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
  ...
  const getSafetyCalculations = () => {
    ...
    let requiredTaper = V <= 60 ? (W * Math.pow(V, 2)) / 155 : (W * V) / 1.6;
  ...
  const triggerAiAudit = () => {
    ...
    const requiredTaper = V <= 60 ? (W * Math.pow(V, 2)) / 155 : (W * V) / 1.6;
    ...
    setTimeout(() => {
      ...
      let critiqueAr = `🤖 **تحليل الذكاء الاصطناعي لمخطط السلامة المرورية (بوابة الأمانة):**\n\n`;
      let critiqueEn = `🤖 **AI Compliance Analysis Report (Amanah Safety Portal):**\n\n`;
      ...
      if (propTerm >= requiredTermination) {
        critiqueAr += `✅ **منطقة نهاية العمل:** الطول المقترح (${propTerm}م) يطابق المعايير البلدية (الحد الأدنى ٣٠م).\n`;
        critiqueEn += `✅ **Termination Taper:** Proposed (${propTerm}m) satisfies the municipal standard minimum of 30 meters.\n`;
      } else {
        critiqueAr += `❌ **منطقة نهاية العمل:** الطول المقترح (${propTerm}م) **أقل** من الحد الأدنى المسموح به للبلدية وهو ٣٠م.\n`;
        critiqueEn += `❌ **Termination Taper:** Proposed (${propTerm}m) is **non-compliant**. The municipal code requires a minimum of 30 meters.\n`;
      }
      ...
      critiqueAr += `\n📝 **توصيات الأمانة العامة:** يرجى إرفاق شهادات مطابقة SASO للمواد العاكسة للتحويلات عند التقديم النهائي.`;
  ...
  const generateLiveAiInsight = async (permit) => {
    ...
    const prompt = language === 'ar'
      ? `أنت نظام "تحكم للذكاء الاصطناعي" (Tahcom AI)، المستشار الهندسي الفني المعتمد لأمانة منطقة المدينة المنورة.
...
4. ⚡ التوصية النهائية المباشرة.`
      : `You are "Tahcom AI", the Official Engineering Auditor for Amanah Al-Madinah.
...
Format the response strictly as a compact visual dashboard using ASCII boxes, progress indicators, and bullet badges for KSA MOT compliance rating.`;
  ...
  const handleSaveInspectionVisitNote = async (permitId, noteText) => {
    ...
    try {
      const res = await fetch(`${API_URL}/api/permits/${permitId}`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
  ...
  const downloadOfficialPermitFile = (permit) => {
    ...
    if (permit.externalPermitDocument) {
      ...
      const content = `===============================================================
أمانة منطقة المدينة المنورة - وكالة التعمير والمشاريع
AMANAH AL-MADINAH MUNICIPALITY - OFFICIAL DETOUR & EXCAVATION PERMIT
===============================================================
...
حالة الاعتماد البلدي: ${permit.status || 'معتمد - APPROVED'}
...
ملاحظات المفتش البلدي:
${permit.inspector_notes || 'تمت المراجعة والتحقق من اشتراطات اللائحة التنفيذية لسلامة التحويلات المرورية بالأمانة.'}

===============================================================
تنبيه: هذا المستند رسمي وصادر من المنظومة الرقمية لأمانة منطقة المدينة المنورة
===============================================================`;
  ...
  if (!currentUser) {
    return <AuthScreen onLogin={setCurrentUser} language={language} t={t} />;
  }
        ...
        <div className="max-w-4xl mx-auto bg-brand-light-card border border-brand-primary/10 rounded-2xl shadow-2xl overflow-hidden p-8 animate-fade-in">
          ...
          <div className="flex flex-col items-center text-center pb-8 border-b border-slate-200">
            ...
            <p className="text-brand-text-gray mt-2 max-w-xl">
              {language === 'ar' 
                ? 'تم إرسال مخطط التحويلة المرورية البلدية وصياغته بنجاح. يمكنك معاينة التقرير ومطابقته أدناه.'
  ...
  return (
    <div className="min-h-screen bg-brand-light text-brand-text-dark py-10 px-4 sm:px-6 lg:px-8 font-sans transition-all duration-300 animate-fade-in relative" dir={t.dir}>
      ...
      <div className="max-w-5xl mx-auto bg-brand-light-card border border-brand-primary/10 rounded-2xl shadow-2xl overflow-hidden">
        ...
        <div className="bg-gradient-to-r from-brand-dark via-brand-primary to-brand-dark px-8 py-8 border-b-2 border-brand-gold/30">
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
            ...
            <div className="flex items-center gap-3">
                    ...
                    <div className="flex items-center justify-between pb-3 border-b border-slate-700 mb-3">
                      <div className="flex items-center gap-2">
                        ...
                        <h4 className="font-bold text-xs text-white">
                          {language === 'ar' ? 'مركز التنبيهات والتوجيهات البلدية' : 'Municipal Directives & Notifications'}
                        </h4>
                      </div>
                      <button
                        type="button"
                          ...
                          <div key={permit.id} className="p-3 bg-slate-800 border border-slate-700 rounded-xl space-y-1.5 hover:border-brand-gold/50 transition">
                            <div className="flex items-center justify-between">
                              ...
                              <span className={`text-[9px] font-bold px-2 py-0.5 rounded-full ${permit.status === 'Rejected' ? 'bg-red-500/20 text-red-400 border border-red-500/30' : 'bg-amber-500/20 text-amber-400 border border-amber-500/30'}`}>
                                {permit.status === 'Rejected' ? (language === 'ar' ? 'مرفوض' : 'Rejected') : (language === 'ar' ? 'توجيه بلدي' : 'Directive')}
                              </span>
                            </div>
              ...
              <div className="flex items-center gap-2.5 bg-brand-dark-hover/60 px-3.5 py-1.5 rounded-xl border border-brand-gold/30 text-xs font-bold text-brand-gold shadow">
                ...
                <span>{currentUser?.username} ({currentUser?.role === 'inspector' ? (language === 'ar' ? 'مفتش الأمانة' : 'Amanah Inspector') : (language === 'ar' ? 'مقاول معتمد' : 'Contractor')})</span>
                <button 
          ...
          {/* Tahcom AI API Key configuration drawer in inspector dashboard */}
          <div className="mt-6 flex flex-col sm:flex-row items-start sm:items-center justify-between bg-brand-dark-hover/30 p-3 rounded-lg border border-brand-gold/15 gap-2">
            <div className="flex items-center gap-1.5 text-xs text-brand-light/80">
              <Settings className="h-4 w-4 text-brand-gold" />
              <span>{language === 'ar' ? 'مفتاح ربط نظام تحكم للذكاء الاصطناعي (Tahcom AI):' : 'Configure Tahcom AI API Key:'}</span>
            </div>
            <input
              type="password"
              placeholder={language === 'ar' ? 'ألصق مفتاح تحكم AI (AIZA...) هنا للربط المباشر...' : 'Paste your Tahcom AI Key here...'}
              value={googleApiKey}
              onChange={handleApiKeyChange}
              className="w-full sm:w-64 bg-slate-900 border border-brand-gold/25 text-xs text-white rounded p-2 focus:outline-none focus:border-brand-gold"
            />
          </div>
                      ...
                      <div>
                        <div className="flex items-center gap-2">
                          <span className="font-extrabold text-xs text-red-700 uppercase tracking-wide">
                            {language === 'ar' ? `آخر تحديث بلدي عاجل — تصريح رقم #${latestPermit.id}` : `Latest Municipal Alert — Permit #${latestPermit.id}`}
                          </span>
                        ...
                        <p className="text-xs text-red-950 font-sans mt-1.5 bg-white/90 p-3 rounded-xl border border-red-200 leading-relaxed font-semibold shadow-sm">
                          <span className="font-extrabold text-red-700">{language === 'ar' ? 'توجيه وملاحظات مفتش الأمانة الأخيرة:' : 'Latest Inspector Directives:'} </span>
                  ...
                  <div className="flex items-center justify-between pb-4 border-b border-slate-200 mb-6">
                    <div className="flex items-center gap-2">
                      ...
                      <h2 className="font-extrabold text-slate-900 text-base">
                        {language === 'ar' ? 'سجل طلبات الترخيص والتوجيهات البلدية' : 'Contractor Permits & Directives Log'}
                      </h2>
                    </div>
                        ...
                        <div key={permit.id} className="p-4 rounded-xl border border-slate-200 bg-slate-50 hover:bg-white transition shadow-sm space-y-3">
                          ...
                          {permit.inspector_notes && (
                            <div className="p-3 bg-red-50 border border-red-200 rounded-lg text-xs">
                              <span className="font-bold text-red-800 block mb-0.5">{language === 'ar' ? 'ملاحظات وتوجيهات مفتش الأمانة:' : 'Inspector Directives:'}</span>
              ...
              <div className="space-y-6 animate-fade-in">
                    ...
                    <div className="p-5 grid grid-cols-1 md:grid-cols-2 gap-5 animate-fade-in">
                          ...
                          return (
                            <div className="col-span-1 md:col-span-2 bg-amber-50 border border-amber-300 rounded-xl p-3.5 text-xs text-amber-900 font-medium space-y-1 mt-2">
                              <div className="flex items-center gap-2 font-bold text-amber-800">
                                ...
                                <span>
                                  {language === 'ar'
                                    ? `مدة العمل المحسوبة: ${Math.round(durationHours)} ساعة (تتجاوز ٧٢ ساعة) — متطلبات المعايير البلدية الإلزامية:`
                                    : `Work Duration: ${Math.round(durationHours)} Hours (Exceeds 72-Hour Threshold) — Enforced Municipal Rules:`}
                                </span>
                              </div>
                      ...
                      <div className="col-span-1 md:col-span-2 border-t border-slate-100 pt-4 mt-2">
                        ...
                        <p className="text-[11px] text-slate-500 mb-2">
                          {language === 'ar' 
                            ? 'نظراً لأن تصريح الحفر صادر من جهة خارجية مستقلة، يرجى رفع نسخة المستند الرسمي الصادر لتتم مراجعته والتحقق من التواريخ المعتمدة بواسطة مفتش الأمانة.' 
                            : 'Because the permit is issued by an external provider, please upload the official permit file for municipal inspector verification.'}
                        </p>
                              ...
                              <div className="flex items-center gap-2">
                                ...
                                <div className="text-left">
                                  ...
                                  <span className="text-[10px] text-emerald-700 font-normal">
                                    {language === 'ar' ? 'تم رفع المستند الرسمي بنجاح - جاهز للمراجعة البلدية' : 'External permit file uploaded successfully'}
                                  </span>
                                </div>
                              </div>
                              <button
                ...
                <div className="bg-white border border-slate-200 rounded-xl overflow-hidden shadow-sm">
                  <button
                    ...
                    className={`w-full flex items-center justify-between p-5 border-b border-slate-200 font-bold text-sm transition-all ${
                      !isPanelComplete('p1_general')
                        ? 'bg-slate-100 text-slate-400 cursor-not-allowed opacity-60' 
                        : 'bg-slate-50 text-brand-dark hover:bg-slate-100/50'
                    }`}
                  >
                    <span className="flex items-center gap-2">
                      <Shield className={`h-4.5 w-4.5 ${!isPanelComplete('p1_general') ? 'text-slate-400' : 'text-brand-primary'}`} />
                      {language === 'ar' ? '١.٢ تجهيزات سلامة الطرق الشريانية وإدارة إضاءة التحويلة' : '1.2 Arterial Road Safety & Smart Detour Lighting'}
                      {!isPanelComplete('p1_general') && <Shield className="h-3.5 w-3.5 text-red-500 animate-pulse" />}
                    </span>
                      ...
                      <div className="p-4 bg-slate-50 border border-slate-200 rounded-xl space-y-4">
                        <div className="flex items-start gap-3">
                          ...
                          <div>
                            ...
                            <p className="text-[11px] text-slate-500 mt-0.5">
                              {language === 'ar' 
                                ? 'يجب تزويد إضاءة التحويلة بمستشعرات لاسلكية لإشعارات الأعطال الفورية والانقطاعات مع الربط المباشر بمركز منصة أمانة المدينة لإدارة إضاءة التحويلات.' 
                                : 'Detour lighting must feature wireless outage sensors and real-time integration with Amanah’s dedicated detour lighting management platform.'}
                            </p>
                          </div>
                        </div>
                            ...
                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-xs">
                              ...
                              <div>
                                ...
                                <select
                                  ...
                                  className="w-full mt-1.5 border border-slate-300 rounded p-2 text-xs font-semibold focus:ring-brand-primary"
                                >
                                  <option value="amanah_platform">{language === 'ar' ? 'منصة الأمانة الرقمية لإدارة إضاءة التحويلات (Amanah Lighting API)' : 'Amanah Dedicated Detour Lighting Platform'}</option>
                ...
                <div className="bg-white border border-slate-200 rounded-xl overflow-hidden shadow-sm">
                  <button
                    ...
                    className={`w-full flex items-center justify-between p-5 border-b border-slate-200 font-bold text-sm transition-all ${
                      (!isPanelComplete('p1_general') || !isPanelComplete('p1_equipment'))
                        ? 'bg-slate-100 text-slate-400 cursor-not-allowed opacity-60' 
                        : 'bg-slate-50 text-brand-dark hover:bg-slate-100/50'
                    }`}
                  >
                    <span className="flex items-center gap-2">
                      <Clock className={`h-4.5 w-4.5 ${(!isPanelComplete('p1_general') || !isPanelComplete('p1_equipment')) ? 'text-slate-400' : 'text-brand-primary'}`} />
                      {language === 'ar' ? '١.٣ جدولة مراحل التنفيذ (Gantt Tracker)' : '1.3 Phasing & Gantt Scheduler'}
                      {(!isPanelComplete('p1_general') || !isPanelComplete('p1_equipment')) && <Shield className="h-3.5 w-3.5 text-red-500 animate-pulse" />}
                    </span>
                ...
                <div className="bg-white border border-slate-200 rounded-xl overflow-hidden shadow-sm">
                  ...
                  >
                    <span className="flex items-center gap-2">
                      ...
                      {language === 'ar' ? '١.٤ خريطة الموقع والنقاط (مرحلة التأسيس)' : '1.4 Site Overview & Nodes Mapping'}
                    </span>
              ...
              <div className="space-y-6 animate-fade-in">
                ...
                <div className="bg-white border border-slate-200 rounded-xl overflow-hidden shadow-sm">
                  <button
                    ...
                    className={`w-full flex items-center justify-between p-5 border-b border-slate-200 font-bold text-sm transition-all ${
                      (!isPanelComplete('p2_blueprint'))
                        ? 'bg-slate-100 text-slate-400 cursor-not-allowed opacity-60' 
                        : 'bg-slate-50 text-brand-dark hover:bg-slate-100/50'
                    }`}
                  >
                    <span className="flex items-center gap-2">
                      <Maximize2 className={`h-4.5 w-4.5 ${(!isPanelComplete('p2_blueprint')) ? 'text-slate-400' : 'text-brand-primary'}`} />
                      {language === 'ar' ? '٢.٢ محرر المقطع العرضي (Cross-Section)' : '2.2 2D Cross-Section Editor'}
                      {(!isPanelComplete('p2_blueprint')) && <Shield className="h-3.5 w-3.5 text-red-500 animate-pulse" />}
                    </span>
              ...
              <div className="space-y-6 animate-fade-in">
                ...
                <div className="bg-white border border-slate-200 rounded-xl overflow-hidden shadow-sm">
                  <button
                    ...
                    className={`w-full flex items-center justify-between p-5 border-b border-slate-200 font-bold text-sm transition-all ${
                      !isPanelComplete('p3_calculator')
                        ? 'bg-slate-100 text-slate-400 cursor-not-allowed opacity-60' 
                        : 'bg-slate-50 text-brand-dark hover:bg-slate-100/50'
                    }`}
                  >
                    <span className="flex items-center gap-2">
                      <Shield className={`h-4.5 w-4.5 ${!isPanelComplete('p3_calculator') ? 'text-slate-400' : 'text-brand-primary'}`} />
                      {language === 'ar' ? '٣.٢ إشغالات ومخطط الإغلاق والحركة' : '3.2 Closures & Traffic Management Plan'}
                      {!isPanelComplete('p3_calculator') && <Shield className="h-3.5 w-3.5 text-red-500 animate-pulse" />}
                    </span>
                        ...
                        <div className="bg-slate-900 border border-slate-700 rounded-xl p-4 text-white text-xs space-y-3">
                          <div className="flex items-center justify-between">
                            <span className="flex items-center gap-2 font-bold text-amber-400">
                              ...
                              {language === 'ar' ? 'إلزامية نوع الحواجز (محرك المعايير البلدية)' : 'Deterministic Barrier Selection (Enforced)'}
                            </span>
                ...
                {/* 3. Gap 4: Pre-Approval Technical Review Checklist (17 Items) */}
                <div className="bg-white border border-slate-200 rounded-xl overflow-hidden shadow-sm">
                  <button
                    type="button"
                    onClick={() => togglePanel('p3_preapproval')}
                    className="w-full flex items-center justify-between p-5 bg-slate-50 border-b border-slate-200 font-bold text-brand-dark text-sm"
                  >
                    <span className="flex items-center gap-2">
                      <CheckSquare className="h-4.5 w-4.5 text-brand-primary" />
                      {language === 'ar' ? '٣.٣ قائمة التحقق المسبق الفنية (١٧ عنصر زامي)' : '3.3 Pre-Approval Technical Checklist (17 Items)'}
                    </span>
                    {expandedPanels.p3_preapproval ? <ChevronUp className="h-4 w-4" /> : <ChevronDown className="h-4 w-4" />}
                  </button>

                  {expandedPanels.p3_preapproval && (
                    <div className="p-5 animate-fade-in">
                      <PreApprovalChecklist language={language} formData={formData} calcs={calcs} readOnly={activePortalMode === 'contractor'} />
                    </div>
                  )}
                </div>

                {/* 4. Gap 4: Field Inspection & Readiness Checklist (20 Items) */}
                <div className="bg-white border border-slate-200 rounded-xl overflow-hidden shadow-sm">
                  <button
                    type="button"
                    onClick={() => togglePanel('p3_fieldinspection')}
                    className="w-full flex items-center justify-between p-5 bg-slate-50 border-b border-slate-200 font-bold text-brand-dark text-sm"
                  >
                    <span className="flex items-center gap-2">
                      <HardHat className="h-4.5 w-4.5 text-amber-500" />
                      {language === 'ar' ? '٣.٤ قائمة التفتيش الميداني والجاهزية (٢٠ عنصر للمفتش)' : '3.4 Field Inspection & Readiness Checklist (20 Items)'}
                    </span>
                    {expandedPanels.p3_fieldinspection ? <ChevronUp className="h-4 w-4" /> : <ChevronDown className="h-4 w-4" />}
                  </button>

                  {expandedPanels.p3_fieldinspection && (
                    <div className="p-5 bg-slate-950 rounded-b-xl animate-fade-in">
                      <FieldInspectionChecklist language={language} onComplete={(results) => console.log('Inspection completed:', results)} />
                    </div>
                  )}
                </div>
                ...
                <div className="bg-white border border-slate-200 rounded-xl overflow-hidden shadow-sm">
                  ...
                  >
                    <span className="flex items-center gap-2">
                      ...
                      {language === 'ar' ? '٣.٥ توثيق الصور الميدانية المكانية (تباعد ≤ ٢٥م)' : '3.5 Geotagged Photo Intake (≤ 25m Spatial Interval)'}
                    </span>
                  ...
                  {expandedPanels.p3_photos && (
                    <div className="p-5 animate-fade-in">
                      <PhotoUploadModule language={language} detourNodes={detourNodes} />
                    </div>
                  )}
                </div>

                {/* 6. Gap 5: Multi-Party Approval Workflow & E-Signatures */}
                <div className="bg-white border border-slate-200 rounded-xl overflow-hidden shadow-sm">
                  <button
                    type="button"
                    onClick={() => togglePanel('p3_approval_workflow')}
                    className="w-full flex items-center justify-between p-5 bg-slate-50 border-b border-slate-200 font-bold text-brand-dark text-sm"
                  >
                    <span className="flex items-center gap-2">
                      <FileCheck className="h-4.5 w-4.5 text-emerald-600" />
                      {language === 'ar' ? '٣.٦ دورة الاعتماد وتوقيعات أطراف التحويلة الـ ٥' : '3.6 Multi-Party 5-Stage Approval & Minutes E-Signatures'}
                    </span>
                    {expandedPanels.p3_approval_workflow ? <ChevronUp className="h-4 w-4" /> : <ChevronDown className="h-4 w-4" />}
                  </button>

                  {expandedPanels.p3_approval_workflow && (
                    <div className="p-5 bg-slate-950 rounded-b-xl animate-fade-in">
                      <ApprovalWorkflow 
                        language={language} 
                        permitId={1} 
                        currentUserRole={activePortalMode} 
                        readOnly={activePortalMode === 'contractor'} 
                        approvalChain={[
                          { role: 'contractor', labelAr: 'مندوب المقاول', labelEn: 'Contractor Representative', status: 'approved', signedBy: formData.projectManagerAr || 'م. طارق الحربي', signedAt: new Date().toISOString().split('T')[0], notes: 'تم استيفاء جميع إجراءات السلامة وتأمين مسار التحويلة.' },
                          { role: 'consultant', labelAr: 'الاستشاري المشرف', labelEn: 'Supervising Consultant', status: 'approved', signedBy: 'د. عبدالمجيد الغامدي', signedAt: new Date().toISOString().split('T')[0], notes: 'تمت مراجعة الاعتبارات الهندسية واختبارات الجسور المعدنية.' },
                          { role: 'safety_dept', labelAr: 'مندوب إدارة السلامة', labelEn: 'Safety Department Delegate', status: 'pending', signedBy: '', signedAt: '', notes: '' },
                          { role: 'maintenance_contractor', labelAr: 'مقاول الصيانة', labelEn: 'Maintenance Contractor', status: 'pending', signedBy: '', signedAt: '', notes: '' },
                          { role: 'maintenance_consultant', labelAr: 'استشاري الصيانة', labelEn: 'Maintenance Consultant', status: 'pending', signedBy: '', signedAt: '', notes: '' }
                        ]}
                        onApprove={(role, notes) => alert(`Role ${role} approved with note: ${notes}`)}
                        onReject={(role, notes) => alert(`Role ${role} rejected with note: ${notes}`)}
                      />
                    </div>
                  )}
                </div>
          ...
          <div className="p-8 space-y-6">
            ...
            <div className="bg-white border border-slate-200 rounded-xl p-4 shadow-sm space-y-4">
              <div className="flex justify-between items-center border-b border-slate-200 pb-3">
                <h2 className="text-base font-extrabold text-brand-dark flex items-center gap-1.5">
                  ...
                  {language === 'ar' ? 'لوحة المراجعة والتدقيق البلدي لتراخيص التحويلات المرورية' : 'Municipal Detour License Review Console'}
                </h2>
              </div>
              ...
              <div className="flex border-b border-slate-200 gap-6 text-xs font-bold overflow-x-auto">
                ...
                >
                  <FileText className="w-4 h-4" />
                  <span>{language === 'ar' ? '📥 الطلبات قيد المراجعة والقرار (Pending)' : '📥 Pending Review & Decision'}</span>
                ...
                >
                  ...
                  <span>{language === 'ar' ? '📜 أرشيف القرارات والتراخيص المعالجة (History)' : '📜 Decision History & Archive'}</span>
                ...
                >
                  ...
                  <span>{language === 'ar' ? '🚧 التحويلات المعتمدة النشطة (Active Zones)' : '🚧 Active Approved Work Zones'}</span>
                    ...
                    <div className="mt-6 bg-white border border-slate-200 rounded-xl overflow-hidden shadow-sm">
                      ...
                      <div className="p-4 space-y-4">
                        ...
                        <div className="flex flex-wrap gap-3">
                          ...
                          <button 
                            onClick={() => handleUpdatePermitStatus('Approved')}
                            className="flex-1 bg-emerald-500 hover:bg-emerald-600 text-white font-bold py-2 px-3 rounded transition-colors flex items-center justify-center gap-1.5 text-xs shadow"
                          >
                            <CheckCircle2 className="w-4 h-4" />
                            {language === 'ar' ? 'اعتماد التصريح' : 'Approve Permit'}
                          </button>
                          <button 
                ...
                <div className="flex justify-between items-center bg-amber-50 border border-amber-200 p-4 rounded-xl shadow-sm">
                  <div>
                    ...
                    <p className="text-xs text-amber-700 mt-0.5">
                      {language === 'ar' 
                        ? 'يتضمن جميع القرارات السابقة الصادرة من المفتشين البلدية (سواءً بالاعتماد أو الرفض أو التوجيهات) مع إمكانية طباعة وتحميل ملفات التصاريح.' 
                        : 'Contains all past municipal inspector decisions (Approved, Rejected, or Directives Issued) with complete permit file download options.'}
                    </p>
                  </div>
            ...
            {inspectorTab === 'active_approved' && (
              <div className="space-y-6 animate-fade-in">
                <div className="flex justify-between items-center bg-emerald-50 border border-emerald-200 p-4 rounded-xl">
                  <div>
                    ...
                    <p className="text-xs text-emerald-700 mt-0.5">
                      {language === 'ar' 
                        ? 'تتيح للمفتش البلدي متابعة أعمال التحويلات القائمة، وتسجيل زيارات الملاحظات الميدانية، والتحقق من التزام المقاول بالتاريخ والتجهيزات.' 
                        : 'Allows municipal inspectors to monitor active detours in real-time, record field audit visits, and verify compliance until work completion.'}
                    </p>
                  </div>
                ...
                {submittedPermits.filter(p => p.status === 'Approved').length === 0 ? (
                  <div className="bg-white border border-slate-200 rounded-xl p-12 text-center text-xs text-slate-400 space-y-2 shadow-sm">
                    <Shield className="w-8 h-8 text-slate-300 mx-auto" />
                    <p>{language === 'ar' ? 'لا يوجد حالياً تحويلات معتمدة نشطة قيد التنفيذ.' : 'No active approved work zones currently in progress.'}</p>
                  </div>
                ) : (
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    {submittedPermits.filter(p => p.status === 'Approved').map(permit => {
                      ...
                      return (
                        <div key={permit.id} className="bg-white border border-slate-200 rounded-xl p-5 shadow-sm hover:shadow-md transition space-y-4">
                          ...
                          {/* Specs & Safety Status */}
                          <div className="grid grid-cols-3 gap-2 text-xs text-slate-700 bg-slate-50 p-2.5 rounded-lg border border-slate-200">
                            <div>
                          ...
                          <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 pt-2 border-t border-slate-100">
                            ...
                            >
                              ...
                              <span>{language === 'ar' ? 'تفتيش دوري TDP-FU' : 'Form TDP-FU Audit'}</span>
                            </button>
                            ...
                            <button
                              onClick={() => {
                                setSelectedPermitForReview(permit);
                                setInspectReportModalOpen(true);
                              }}
                              className="py-1.5 bg-slate-100 hover:bg-slate-200 text-slate-800 font-bold text-xs rounded transition flex items-center justify-center gap-1"
                            >
                              <FileCheck className="w-3.5 h-3.5 text-brand-primary" />
                              <span>{language === 'ar' ? 'معاينة الشهادة' : 'View Certificate'}</span>
                            </button>
                          </div>
                        </div>
                      );
        ...
        {/* Active Detour Form TDP-FU Periodic Monitoring Modal */}
        <ActiveDetourMonitoringModal
          ...
          language={language}
          onSaveAudit={handleSavePeriodicAudit}
        />

        {/* Detour Closure & Site Restoration Form TDP-CL Modal */}
        <DetourClosureModal
          ...
          language={language}
          onCompleteClosure={handleSaveClosureMinutes}
        />
