...
const dict = {
  ar: {
    dir: 'rtl',
    portalTitle: 'نظام تخطيط وتراخيص السلامة المرورية',
  ...
  en: {
    dir: 'ltr',
    portalTitle: 'Traffic Safety & Permit Licensing Portal',
...
const ConstructionPlanningInterface = () => {
  ...
  const handleUpdatePermitStatus = async (status) => {
    ...
    try {
      const res = await fetch(`http://localhost:5000/api/permits/${targetId}`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
  ...
  const handleExportCAD = async () => {
    ...
    try {
      ...
      const res = await fetch('http://localhost:5000/api/generate-cad', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          lat: centerLat,
  ...
  const handleSavePeriodicAudit = async (auditData) => {
    try {
      await fetch(`http://localhost:5000/api/permits/${auditData.permitId}/periodic-inspections`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
  ...
  const handleSaveClosureMinutes = async (closureData) => {
    try {
      await fetch(`http://localhost:5000/api/permits/${closureData.permitId}/closure`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
      ...
      await fetch(`http://localhost:5000/api/permits/${closureData.permitId}`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
  ...
  const handleSaveFieldReadiness = async (readinessData) => {
    try {
      await fetch(`http://localhost:5000/api/permits/${readinessData.permitId}/field-readiness`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
  ...
  const handleSaveOpeningMinutes = async (openingData) => {
    try {
      await fetch(`http://localhost:5000/api/permits/${openingData.permitId}/opening-minutes`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
...
Format the response strictly as a compact visual dashboard using ASCII boxes, progress indicators, and bullet badges for KSA MOT compliance rating.`;
  ...
  const handleSaveInspectionVisitNote = async (permitId, noteText) => {
    ...
    try {
      const res = await fetch(`http://localhost:5000/api/permits/${permitId}`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
...
===============================================================`;
  ...
  if (!currentUser) {
    return <AuthScreen onLogin={(user) => { setShowHomeScreen(true); setCurrentUser(user); }} language={language} t={t} />;
  }
                    ...
                    <div className="grid grid-cols-1 gap-4">
                      {submittedPermits.map(permit => (
                        <div key={permit.id} className="p-4 rounded-xl border border-slate-200 bg-slate-50 hover:bg-white transition shadow-sm space-y-3">
                          ...
                          {/* Official documents \u2014 view-only for the contractor. These are
                              generated and issued by the inspector, not something the
                              contractor can trigger themselves. */}
                          <div className="pt-2 border-t border-slate-200">
                            <p className="text-[10px] font-bold text-slate-400 uppercase mb-1.5">
                              {language === 'ar' ? 'المحاضر الرسمية (متاحة من جهة المفتش)' : 'Official Documents (available from the inspector side)'}
                            </p>
                            <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
                              <div className="py-1.5 bg-slate-100 text-slate-400 font-bold text-[11px] rounded flex items-center justify-center gap-1 border border-slate-200 cursor-not-allowed" title={language === 'ar' ? 'متاح من جهة المفتش' : 'Available from the inspector side'}>
