...
import { detectSaudiCrs, reprojectCadToWgs84 } from '../utils/coordinateEngine';
...
self.onmessage = async (e) => {
  const { fileContent, fileName, anchorLat = 24.4686, anchorLng = 39.6120, preferredCrs = null } = e.data;

  try {
    self.postMessage({ type: 'progress', progress: 20, message: 'Parsing CAD DXF vector geometry...' });
    ...
    self.postMessage({ type: 'progress', progress: 50, message: 'Reprojecting Saudi UTM grid coordinates...' });
